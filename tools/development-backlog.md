...
| Issue #18: two regional CPUE series | **Implemented, 14 September; final checks/publication in progress.** The separate Regional CPUE indices vignette uses one NB2 glmmTMB interaction model and the existing index API. Six figures, assessment outputs, and native covariance checks demonstrate the fixed regional references. The [design record](two-region-plan.md) separates within-region covariance from cross-region dependence and leaves selective CRA5 prediction out of scope. |
