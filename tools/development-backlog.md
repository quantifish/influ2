...
does not reopen completed issues. Issue #18 was explicitly resumed on
14 September; its implementation and validation are recorded below.
...
| Issue #18: two regional CPUE series | **Completed, 14 September; issue closed.** The separate Regional CPUE indices vignette uses one NB2 glmmTMB interaction model and the existing index API. Six figures, assessment outputs, and native covariance checks demonstrate the fixed regional references. Local archive checks, Ubuntu/Windows checks, coverage, and publication pass. The [design record](two-region-plan.md) separates within-region covariance from cross-region dependence and leaves selective CRA5 prediction out of scope. |
| Assessment tables and annual covariance (#22) | **Completed, 13 September; issue closed.** `index_table()` omits all-missing medians for reporting, preserves the full `as.data.frame()` schema, and adds explicit marginal lognormal parameters. `index_vcov()` supplies compact joint annual covariance. See the [assessment-output audit](assessment-output-review.md) and [release review](release-review.md) for validation and publication status. |
| Remaining legacy helpers and frozen Get Started article | **Parked for later review.** Preserve the current records and article until the maintainer makes the remaining decisions. |
