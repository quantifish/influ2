...
| Assessment tables and annual covariance (#22) | **Approved implementation, 13 September.** `index_table()` omits all-missing medians for reporting, preserves the full `as.data.frame()` schema, and adds explicit marginal lognormal parameters. `index_vcov()` supplies compact joint annual covariance. See the [assessment-output audit](assessment-output-review.md) and [release review](release-review.md) for validation and publication status. |
