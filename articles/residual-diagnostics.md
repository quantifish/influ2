...
operates on the calculated `influ_residuals` object. If you already have
response simulations,
[`as_influ_residuals()`](https://www.quantifish.co.nz/influ2/reference/as_influ_residuals.md)
provides a second route to that same object; see [Using simulations
generated elsewhere](#using-simulations-generated-elsewhere).
...
Panels A-C already use **simulation-based PIT residuals on the
standard-normal scale**: the stored randomised PIT/rank values are
transformed with `qnorm(pit)`. These are the same residuals in the Q-Q,
fitted-value, and year panels, not three different residual definitions.
This transformation provides a familiar reference scale; it does not
guarantee normally distributed residuals from an estimated model.

Panel D is a different, complementary check of the **response**, not an
ECDF of the PIT values. The response is whatever is modelled: for
example, CPUE, catch, or encounter. The figure footer identifies both
types of check explicitly.
...
1.  **Distributional Q-Q:** normal scores of randomised PIT/simulation
...
### Choosing the conditioning

`conditioning = "backend_default"` preserves the table above. The same
plotting interface does not make different predictive targets
interchangeable. Choose an explicit target when comparing models, and
check the recorded scheme:

| Model     | Supported `conditioning` values, besides `"backend_default"` |
|:----------|:-------------------------------------------------------------|
| GLM / GAM | `"fitted"`                                                   |
...
lobster_conditional <- influ_residuals(
  lobster_nb_mixed, nsim = 250, seed = 42, conditioning = "fitted"
)
lobster_conditional$metadata$scheme
#> [1] "Observation simulations conditional on fitted random effects and parameters"
```

`"fitted"` fixes estimated parameters, smooths, and random effects. It
asks about response variation conditional on that estimated structure.
In contrast, `"new_effects"` regenerates random effects at fitted
distribution parameters. For sdmTMB it follows native `re_form = NA`:
fitted smooths remain fixed.

`"conditional_draw"` holds fixed parameters at their estimates and takes
**one joint approximate conditional draw of the latent effects**. That
vector is shared across all response simulations, preserving its spatial
and temporal dependence. It is not a fresh field per simulation, full
Bayesian parameter uncertainty, or MCMC. See the executed comparisons in
[Spatial and spatiotemporal
diagnostics](https://www.quantifish.co.nz/influ2/articles/spatial-spatiotemporal.html#residual-conditioning).

New spatial schemes and glmmTMB `"fitted"` prepare an independent native
objective and per-replicate seeds. Changing `batch_size` therefore
changes neither their latent draw nor their observation-level results.
The compact response-ECDF grid still uses the first batch and can
differ. Neither the objective, latent vector, nor full response matrix
is retained in the result. Backend setup and sparse factorisation can
still use substantial memory. The existing default routes keep their
previous random-number behaviour.

Unsupported combinations fail explicitly. In particular, this interface
does not silently turn brms posterior predictive checks into
fixed-parameter checks, sample GAM coefficient uncertainty, or
regenerate tinyVAST processes. The external-simulation constructor can
describe additional explicitly generated schemes. A shared name does not
guarantee equally calibrated diagnostics: estimated parameters,
approximate latent distributions, and fitted-data reuse still affect the
reference distribution. Do not select a seed because its residuals look
best, or treat a conditional check as full validation of the spatial
covariance structure.

The shared conditional-draw option requires a converged ML fit with a
positive-definite Hessian, not a REML or profiled fit. Some native REML
latent vectors also contain fixed coefficients; sampling them would
contradict the promise to hold those coefficients fixed, so that case is
rejected explicitly.
...
consequently be displaced from the identity line. By default,
sdmTMB/tinyVAST use their existing `mle-eb` conditioning. With
`conditioning = "conditional_draw"`, their bins instead use expected
probabilities at the **same sampled latent vector**, not noisy
binary-simulation averages. With `"new_effects"`, fitted conditional
bins remain distinct from the process-varying simulation envelope. brms
bins use posterior mean expected probabilities over the same draw
identities as the posterior predictive simulations, including existing
group effects.

In particular, **the grey bars are not error bars attached to the purple
points**. Each purple point is the actual observed proportion; each bar
is the range of proportions predicted by the model’s simulations for
that bin. A point outside its bar is therefore possible and informative.
For the fixed- parameter GAM and offset-only examples below, those
ranges generally lie near the diagonal identity line, rather than around
the observed points. They may be asymmetric and vary in width between
bins. In the grouped difference plot, the same simulated ranges are
expressed relative to the predicted proportion, so their reference is
zero instead of the diagonal. We keep separate bars: the stored
summaries do not define a continuous uncertainty region between bins.
...
proportions, the dashed line is identity, and grey bars are pointwise
95% ranges of simulated bin proportions under each model’s fixed
probabilities. The bars are model predictions, not error bars around the
purple points: a point outside its bar indicates a discrepancy for that
bin. They are not a continuous band between bins or a calibrated
pass/fail test. The distorted probabilities show systematic departures,
rather than merely a different overall encounter rate.
...
encounter proportions; grey bars are pointwise 95% ranges of
simulated-minus-fitted proportions, conditional on the fixed fitted
probabilities. These are model-predicted ranges around the zero
reference, not error bars around the observed purple points, which may
lie outside them. A pooled calibration pass is not an all-clear for
model structure.
...
pcod_checks <- influ_residuals(
  pcod_model, nsim = 250, seed = 41, conditioning = "conditional_draw"
)
plot(pcod_checks, type = "qq")
```

This fixes one native approximate conditional latent draw across all
response simulations and batches, as recorded in
`pcod_checks$metadata$scheme`. It uses finite-simulation randomised
ranks rather than sdmTMB’s analytic quantile CDF, so the two results are
not numerically identical. Omit `conditioning` or choose `"fitted"` to
retain the previous fitted-field route; use `"new_effects"` for native
process simulations with fitted smooths fixed. The identity reference
preserves location and scale departures instead of fitting a quartile
line through the points. These remain exploratory fitted-model
diagnostics.
...
simulated normal-score ranks. Fitted-field conditioning remains the
default, with one shared conditional latent draw now available
explicitly:

``` r

tiny_checks <- influ_residuals(
...
single-draw residual recipe. influ2’s new `"conditional_draw"` option
deliberately prepares one native TMB draw and reuses it, rather than
calling tinyVAST `"mle-mvn"` separately for each batch. This does not
provide an automatic multivariate DHARMa adapter or establish universal
...
## Optional PIT ECDFs and choosing the four panels

The normal-score PIT transformation is already implemented; it is not an
outstanding feature. A PIT ECDF offers a different display of those
**same stored ranks**. It compares their cumulative distribution with
the uniform reference. The difference version subtracts the reference,
`ECDF(u) - u`, making departures visible around a horizontal zero line.
Neither is the observed-response ECDF from the default fourth panel, a
new residual method, or a LOO-PIT calculation.

These optional panels use
[`bayesplot::ppc_pit_ecdf()`](https://mc-stan.org/bayesplot/reference/PPC-distributions.html)
with supplied PIT values. Install **bayesplot \>= 1.16.0** to use them.
The default overview does not require bayesplot. Nothing below refits a
...
patchwork::wrap_plots(
  plot(nb_checks, type = "pit_ecdf"),
  plot(nb_checks, type = "pit_ecdf_diff"),
  ncol = 2
)
```

![Side-by-side PIT ECDF and difference plots for the same saved glmmTMB
residual
object.](residual-diagnostics_files/figure-html/residual-pit-ecdfs-1.png)

Two displays of the existing simulation-based PIT values for the
negative-binomial glmmTMB model: their ECDF (left) and ECDF minus the
uniform reference (right). Dark solid curves show the empirical
distribution or its difference; pale dashed curves are bayesplot’s
numerically adjusted 95% simultaneous reference limits assuming
independent uniform PIT values. Grey dotted lines mark identity and
zero, respectively. These are exploratory reference limits, not
calibrated goodness-of-fit thresholds for this fitted mixed model. No
...
chooses response calibration or a response ECDF using the original
response metadata. For example, retain the response ECDF and replace the
year panel with a PIT ECDF difference:

``` r

plot(nb_checks, panels = c("qq", "fitted", "pit_ecdf_diff", "distribution"),
  response_scale = "log1p")
```

![Custom four-panel overview with a PIT ECDF difference replacing the
year
boxplots.](residual-diagnostics_files/figure-html/residual-select-panels-1.png)

A customised four-panel diagnostic retaining the Q-Q, fitted-value, and
response-distribution checks, with a PIT ECDF difference in the
lower-left position. Panels A and B use normal-score PIT residuals,
panel C uses the original PIT values on a uniform horizontal scale, and
panel D compares response distributions. The footer follows the selected
panel order. This is an optional layout; the default still includes the
year panel.

All standalone types are available in `panels`, including
`"calibration"` and `"calibration_groups"` when the needed summaries
were calculated. There must be exactly four names; their order and
repetition are allowed. Missing summaries produce an informative error
rather than silently substituting a different diagnostic. For an
optional PIT plot in the fourth position, use:

``` r

plot(nb_checks, panels = c("qq", "fitted", "year", "pit_ecdf"))
```

That last layout omits the complementary response check, so inspect it
separately with `plot(nb_checks, type = "distribution")` when needed.
Selecting panels does not change the fitted model, saved residuals, or
simulation scheme.

## Using simulations generated elsewhere

You may already have simulations from a bespoke model, a collaborator’s
workflow, or a native package call.
[`as_influ_residuals()`](https://www.quantifish.co.nz/influ2/reference/as_influ_residuals.md)
calculates the same compact residual diagnostic from those supplied
responses. It does not fit a model, run MCMC, or generate more
responses. Both entry points use the same rank, predictive-mean, ECDF,
and calibration calculation engine.

The matrix must have **one observation per row and one complete joint
simulation per column**. Supply matching observation data, a response
column, an explicit year column, a response kind, and a description of
the simulation conditioning. Matrix row names must exactly match the
data’s row names, or a declared `observation_id` column, in the same
order. Missing, duplicated, or reordered IDs fail explicitly. Nothing is
silently transposed or realigned. The labels must come from verified
native output order; assigning plausible row names cannot establish that
the underlying observations really match.

For a worked example, reuse the fitted lobster glmmTMB model from the
start of this article. The first call below generates 100 whole-response
replicates using glmmTMB itself. It simulates observation variation and
redraws the monthly random effects at fitted parameters. The constructor
then only summarises that existing matrix:

``` r

set.seed(20260912)
response_simulations <- as.matrix(simulate(lobster_nb_mixed, nsim = 100))
fitted_rows <- rownames(model.frame(lobster_nb_mixed))
data_rows <- match(fitted_rows, rownames(lobsters_per_pot))
stopifnot(!anyNA(data_rows), nrow(response_simulations) == length(fitted_rows))
simulation_data <- lobsters_per_pot[data_rows, , drop = FALSE]
# Native simulate() follows the fitted observation order for this count model.
rownames(response_simulations) <- fitted_rows

external_checks <- as_influ_residuals(
  response_simulations, simulation_data,
  response = "lobsters", year = "year", response_kind = "distribution",
  conditioning = "Fitted parameters; monthly random effects resimulated",
  groups = "month", batch_size = 25, seed = 41
)
external_checks
#> Simulation-based residual diagnostics (external)
#> 5049 observations; 100 simulations
#> Time: year [ explicit external column ]
#> Fitted parameters; monthly random effects resimulated
#> Exploratory ranks; not a calibrated goodness-of-fit test
```

``` r

plot(external_checks, response_scale = "log1p")
```

![Four-panel residual overview using externally supplied glmmTMB lobster
simulations.](residual-diagnostics_files/figure-html/residual-external-overview-1.png)

The standard four-panel diagnostic calculated from 100 externally
generated whole-response simulations of the fitted lobster glmmTMB
model. Each native simulation includes newly simulated monthly random
effects and observation variation at fitted parameters. The constructor
generates no further responses, and its predictive means come from this
same supplied matrix. These are different replicates from the earlier
overview, so their ranks and Monte Carlo summaries need not match it
exactly. The normal-score PIT panels and response ECDF retain their
usual exploratory interpretation; the ribbons are not calibrated
mixed-model tests.

The returned object also works with the standalone Q-Q/PIT views and the
existing residual helpers. The grouping columns must be retained during
...
retain it, the fitted model, or a function capturing either. The matrix
you created still occupies memory until you release it. This initial
interface is not an on-disk reader or streaming simulator. For large
supported fits, the native
[`influ_residuals()`](https://www.quantifish.co.nz/influ2/reference/influ_residuals.md)
route can avoid creating the full response matrix in the first place.
Plotting an existing diagnostic repeats neither calculation.

For a fixed matrix, the seed controls tie randomisation, not response
simulation. Ranks and predictive means are unchanged by the calculation
batch size; the compact response ECDF grid can change because it
includes the first batch’s range. A fitted-model call may consume random
numbers while selecting posterior draws before randomising ranks, so the
same seed alone does not guarantee the same ranks across the two entry
points.

### Response components and encounter probabilities

The declared response kind determines which response check is available;
it is not guessed from a sample of zeros and ones. Use `"combined"` for
a joint delta/hurdle/zero-inflated response. To diagnose a positive
component, supply its own observations and complete component
simulations, and declare `component = "positive"`. The constructor never
filters simulated zeros or converts a combined matrix into a
positive-component diagnostic. The caller must verify that the declared
component and simulation target are correct.

Bernoulli and known-trial binomial inputs require an additional column
of **original fitted success probabilities**, not the average of
simulated zero/ one responses. Record their conditioning separately from
the response simulations. For example, given already aligned encounter
data and simulations:

``` r

encounter_checks <- as_influ_residuals(
  simulated_presence, matched_data,
  response = "present", year = "year", response_kind = "bernoulli",
  component = "encounter", probability = "fitted_probability",
  conditioning = "Observation error at fitted parameters and smooths",
  probability_conditioning = "Fitted parameters and smooths",
  calibration_groups = c("year", "target")
)
plot(encounter_checks)
```

For `response_kind = "grouped_binomial"`, supply integer success counts
and a `trial_counts` column containing the known numbers of trials. Its
default response panel remains the ECDF, with calibration available
explicitly. No case weights or area weights are interpreted as trials.
The help page for
[`as_influ_residuals()`](https://www.quantifish.co.nz/influ2/reference/as_influ_residuals.md)
sets out the full input and alignment contract.
