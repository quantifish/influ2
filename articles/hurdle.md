...
The vignette uses the pre-fitted object supplied with its source. Joint
...
When inspecting a term with
`plot(diagnostic, type = "cdi", term = ...)`, select one `component` at
a time. The top panel centres that component’s effect on the same
reference as its influence. Positive log-link and lognormal effects
appear as ratios about one, whereas a logit occurrence or extra-zero
effect remains in centred log-odds about zero. An increase in extra-zero
log-odds has the opposite interpretation to an increase in occurrence
log-odds. Neither is itself an unconditional catch multiplier; use the
combined unconditional-mean influence to assess that quantity.
...
arithmetic-mean ratios, but its dispersion effects are not separately
decomposed. Combined hurdle/zero-inflated diagnostics with offsets or
exposure additions are also rejected until an offset-aware reference
