...
index_plot <- function(fit, year = "Year", fill = "purple", probs = c(0.25, 0.75)) {
  ...
  unstd$cpue <- unstd$cpue / geo_mean(unstd$cpue)
  df1 <- fout %>%
    mutate(Estimate = NA, Est.Error = NA, Q25 = NA, Q50 = unstd$cpue, Q75 = NA, model = "Unstandardised")
  
  df <- rbind(fout, df1)
  df$model <- factor(df$model, levels = c("Unstandardised", "Standardised"))
  
  p <- ggplot(data = df, aes(x = year, y = Q50, group = model)) +
    geom_ribbon(aes(ymin = Q25, ymax = Q75, fill = model), alpha = 0.5, colour = NA) +
...
step_plot <- function(fits, year = "year", probs = c(0.25, 0.75), show_probs = TRUE) {
  ...
  for (i in 1:m) {
    ...
    # fout[[i]]$Estimate <- fout[[i]]$Estimate / geo_mean(fout[[i]]$Estimate)
    ...
    fout[[i]]$Estimate <- fout[[i]]$Q50

    if (i > 2) {
      xx <- fout[[i-2]] %>% mutate(model = fout[[i]]$model, line = i)
      df_grey <- rbind(df_grey, xx)
    }
    if (i > 1) f2 <- fout[[i]] %>% mutate(Estimate = f1$Estimate, colour = "b")
    f1 <- fout[[i]]
  ...
  p <- ggplot(data = df)
  p <- p + geom_line(data = df_grey, aes(x = year, y = Estimate, group = line), colour = "grey", linetype = "solid")
  if (show_probs) {
    p <- p + geom_ribbon(data = df, aes(x = year, ymin = Q25, ymax = Q75, group = colour, fill = colour), alpha = 0.3, colour = NA)
  }
  p <- p + 
    geom_line(data = df, aes(x = year, y = Estimate, colour = colour, group = colour, linetype = colour)) +
    geom_point(data = df, aes(x = year, y = Estimate, colour = colour, shape = colour)) +
