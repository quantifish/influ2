...
iris2 <- iris %>% 
  mutate(CPUE = Petal.Length, Year = factor(Sepal.Width * 10 + 1970), 
         Area = factor(round(Petal.Width)), 
         Duration = Sepal.Length, Duration2 = cut(Sepal.Length, 20)) %>%
...
ggplot(data = rbind(cpue1, cpue2, cpue3), aes(x = Year, y = Median, colour = Type)) +
