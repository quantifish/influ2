...
First I fit three different models of increasing complexity in `brms`. The third model includes the explanatory variables `Year`, `Species` and `Area`. I then fit another four models which include `Year` and different forms of the variable `Duration`. The different forms include a linear predictor, a third-order polynomial, a spline, and a random-effect. A few different distributions are used to mix things up as well including lognormal, Gaussian, Gamma, and Weibull `r getwd()`.
